...
def get_taxa():
    ...
    db.close()
    return [{"guid": t.guid, "genus": t.genus, "species": t.species, "full_name": t.full_name} for t in taxa]

@app.post("/taxa")
def create_taxon(genus: str, species: Optional[str] = None):
    db = SessionLocal()
    now = datetime.now().isoformat()
    full_name = genus + (f" {species}" if species else "")
    taxon = Taxon(genus=genus, species=species, full_name=full_name, created_at=now, updated_at=now)
    db.add(taxon)
    db.commit()
...
@app.put("/taxa/{guid}")
def update_taxon(guid: str, genus: str, species: Optional[str] = None):
    db = SessionLocal()
    ...
    taxon.full_name = genus + (f" {species}" if species else "")
...
def search_taxa(q: str = ""):
    ...
    taxa = db.query(Taxon).filter(
        (Taxon.genus.contains(q)) | (Taxon.species.contains(q)) | (Taxon.full_name.contains(q))
    ).limit(20).all()
    db.close()
    return [{"guid": t.guid, "genus": t.genus, "species": t.species, "full_name": t.full_name} for t in taxa]
...
def search_objects(q: str = "", type: Optional[str] = None, limit: int = 20):
    ...
    if not type or type == "taxon":
        ...
        for t in taxa:
            results.append({"type": "taxon", "guid": t.guid, "name": t.full_name})
    db.close()
    return results
